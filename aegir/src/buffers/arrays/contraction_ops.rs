...
impl_contract!(0; <F> F, F; |self, y| -> F { Ok(self * y) });

impl_contract!(0; <F, N> [F; N], F; |self, y| -> [F; N] { Ok(self.map(|xi| xi * y)) });
impl_contract!(0; <F, N> F, [F; N]; |self, y| -> [F; N] { Ok(y.map(|yi| self * yi)) });
...
impl_contract!(1; <F> F, F; |self, y| -> F { Ok(self * y) });
